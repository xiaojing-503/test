...
from utils.get_response import get_deepseek_response,get_codes_response,get_llm_response,get_gpt_response,get_baidu_response,get_hauweiyun_deepseek
        ...
        for i, item in enumerate(data):
            ...
            if new_err_type=='result':
                ...
                if item['err_type1'] != "correct" and item['err_type2'] != "correct":
                    ...
                    while True:
                        try:
                            sql = get_hauweiyun_deepseek(result_correction_instruction, result_inputs)
                            sql = format_sql_to_single_line(sql)
                            item['sql_new'] = sql
                            print('sql:', sql)
                            break  # 成功获取 SQL，跳出循环
                ...
                # 存在skeleton错误
                    ...
                    while True:
                        try:
                            sql = get_hauweiyun_deepseek(result_correction_instruction_skeleton, result_inputs)
                            sql = format_sql_to_single_line(sql)
                            item['sql_new'] = sql
                            print('sql:', sql)
                            break  # 成功获取 SQL，跳出循环
                ...
                while True:
                    try:
                        ...
                        sql = get_hauweiyun_deepseek(empty_correction_instruction, empty_inputs)
                        sql = format_sql_to_single_line(sql)
                        item['sql_new'] = sql
                        print('sql:', sql)
                        break  # 成功获取 SQL，跳出循环
                ...
                while True:
                    try:
                        sql = get_hauweiyun_deepseek(system_correction_instruction, system_inputs)
                        sql = format_sql_to_single_line(sql)
                        item['sql_new'] = sql
                        print('sql:', sql)
                        break  # 成功获取 SQL，跳出循环
...
input_path = '/root/Schema-Value/DIN-SQL/result/new/bird_dev_resultt.json'
output_path = '/root/Schema-Value/DIN-SQL/result/new/bird_dev_correct3.json'
