...
from utils.get_response import get_deepseek_response,get_codes_response,get_llm_response,get_gpt_response
...
from check.check_system import check_system_with_eval_result
    ...
    with open(corrected_file, 'w', encoding='utf-8') as f:
        ...
        for i, item in enumerate(data):
            ...
            if new_err_type=='result':
                ...
                while True:
                    try:
                        ...
                        sql = get_gpt_response(empty_correction_instruction, empty_inputs)
                        sql = format_sql_to_single_line(sql)
                        item['sql_new'] = sql
                        print('sql:', sql)
                        break  # 成功获取 SQL，跳出循环
                ...
                while True:
                    try:
                        sql = get_gpt_response(system_correction_instruction, system_inputs)
                        sql = format_sql_to_single_line(sql)
                        item['sql_new'] = sql
                        print('sql:', sql)
                        break  # 成功获取 SQL，跳出循环
            ...
            if i < len(data) - 1:
                f.write(",\n")
        f.write("]")  # 写入 JSON 数组的结束符号
bird_path='/root/Schema-Value/RSL-SQL/ours/deepseek_step3.json'
eval_file = '/root/Schema-Value/RSL-SQL/ours/deepseek_step3_eval.json'
BIRD_DATABASE='/root/Text-to-SQL/hdd/BIRD/data/sft_data_collections/bird/dev/dev_databases/'
directory='/root/Schema-Value/RSL-SQL/ours/'
bird_output=os.path.join(directory, 'deepseek_step3_initial.json')
# # system错误
# check_system_with_eval_result(eval_file,bird_path,bird_path)
# # 会验证empty错误
...
bird_resultt = os.path.join(directory, 'bird_dev_resultt.json')
check(bird_result,bird_resultt)
# # # 处理反引号
