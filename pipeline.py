...
import json
import os
from utils.check_sql_result import compare_sql_results,compute_report,check_system_error
...
def identify_error(sql_file,output_file,bird_database,timeout=10):
    ...
    for item in data:
        ...
        if check_system_error(bird_database, db_id, sql_initial, timeout)=='system':
            new_err_type = 'system'
        else:
            skeleton = get_sql_skeleton(sql_initial)  
            sql_schemas = get_sql_schema(sql_initial)  
            values = extract_values(sql_initial) 
            ...
            if check_empty(sql_initial,db_id,bird_database)=='empty':
                new_err_type = 'empty'
...
#                 print('inputs:',inputs)
#                 tokenized_inputs = prepare_inputs(correction_instruction+inputs, tokenizer, max_tokens - max_new_tokens)
...
# 生成初始err_pred1 初始err_pred2
generate_initial_sql()

# 运行evaluate得到一个初始EX （仅检测err_pred1）
# 

# 识别错误 (需要修改: 先识别系统错误, 再识别empty错误, 其他的再生成sql1 sql2,最后识别result)
